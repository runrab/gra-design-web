  ...
  <div class="page-header-index-wide">
    <a-row :gutter="24">
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="用户数量" >
          <span slot="action2">  {{result.userCount | NumberFormat}}</span>
      ...
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="学生数量" >
          <span slot="action2">  {{result.userStuCount| NumberFormat}}</span>
          <a-tooltip title="指标说明" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
      ...
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="填写数量">
          <span slot="action2"> {{result.inCount  | NumberFormat}}</span>
          <a-tooltip title="指标说明" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
      ...
      <a-col :sm="24" :md="12" :xl="6" :style="{ marginBottom: '24px' }">
        <chart-card :loading="loading" title="填写进度" total="66%" >
          <span slot="action2">  {{result.progress | NumberFormat}}</span>
          <a-tooltip title="指标说明" slot="action">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
    ...
    <a-card :loading="loading" :bordered="false" :body-style="{padding: '0'}">
      <div class="salesCard">
        <a-tabs default-active-key="1" size="large" :tab-bar-style="{marginBottom: '24px', paddingLeft: '16px'}">
          ...
          <a-tab-pane loading="true" tab="人数" key="1">
            <a-row>
              <a-col :xl="16" :lg="12" :md="12" :sm="24" :xs="24">
                <bar title="人数排行" :dataSource="barData"/>
              </a-col>
              <a-col :xl="8" :lg="12" :md="12" :sm="24" :xs="24">
                <rank-list title="热门城市排行榜" :list="rankList"/>
              </a-col>
            </a-row>
          </a-tab-pane>
          <a-tab-pane tab="就业趋势" key="2">
            <a-row>
              <a-col :xl="16" :lg="12" :md="12" :sm="24" :xs="24">
                <bar title="人数趋势" :dataSource="barData"/>
              </a-col>
              <a-col :xl="8" :lg="12" :md="12" :sm="24" :xs="24">
                <rank-list title="热门城市排行榜" :list="rankList"/>
              </a-col>
...
<script>
  ...
  //更改为修改版本 单独使用防止影响别处
  import ChartCard from '@/components/ChartCard2'
  ...
  import { getLoginfo,getVisitInfo, showInfoCount } from '@/api/api'
  ...
  export default {
    ...
    methods: {
      initLogInfo () {
        ...
        showInfoCount().then(res=>{
          if(res.success){

            // this.visitInfo = res.result;
            this.result.progress = res.result.progress;
